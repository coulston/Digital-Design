...
\section{Always/Case:  playsLeft}

The module labeled playsLeft interprets the 2-bit input
...
The first few rows for the truth table for playsLeft are shown in
...
You should implement playsLeft inside the top-level hiLow module using the
incomplete always/case statement shown  in Listing~\ref{listing:playsLeft}.

\begin{lstlisting}[language=Verilog,
 caption={Starter code for playsLeft.},
 label={listing:playsLeft},
 frame=single]
 always @(*)
    case (playSwitch)
        2'b00: greenLEDs = 4'bxxxx;    	// 4 guesses left
        default:  hiLowSeg = 4'bxxxx;
    endcase
 \end{lstlisting}

Remember that the output type from an always/case statement must 
have the ``reg'' qualifier, not ``wire''.

\section{Always/Case:' hiLowWin}
...
You should implement hiLowWin inside the top-level hiLow module using the
incomplete always/case statement shown  in Listing~\ref{listing:hiLowWinVerilog}.

\begin{lstlisting}[language=Verilog,
 caption={Starter code for hiLowWin.},
...
and you filled them all in, you would get 14 on the 8\textsuperscript{th} row. Can you figure out what number is excluded
from the sequence?
...
\item
Make the \verb+seedSwitch, playSwitch, guessSwitch+ inputs vectors with the
