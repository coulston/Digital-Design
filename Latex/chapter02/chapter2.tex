...
\item {Step 4: Remove the lowest precedence operation from the expression 
creating zero or more child subexpressions.}
Removing the OR operator from $A*(B+C')+B'C$ yields two subexpressions, 
$A*(B+C')$, and $B'C$.  

\item {Step 5: Parse each child subexpression independently.}
...
complete the parsing process below.
\vspace{0.2cm}

\begin{tabular}{m{5cm} | m{5cm}}
$A*(B+C')$		&			$B'C$ \\ \hline
\textbf{Step 1:}  The lowest precedence
operation in this expression, AND between
$A$ and $B+C'$.  &
\textbf{Step 1:}  The lowest precedence
operation in this expression, AND between
$B'$ and $C$. \\ \hline
\textbf{Step 2:}   Draw the AND gate labeled 2 Figure~\ref{fig:represenationsAdvance}.   &
\textbf{Step 2:}   Draw the AND gate labeled 5 Figure~\ref{fig:represenationsAdvance}.   \\ \hline
\textbf{Step 3:}   Connect the output of AND gate labeled 2 to the input of the OR gate labeled 1. &
\textbf{Step 3:}   Connect the output of AND gate labeled 5 to the input of the OR gate labeled 1. \\ \hline
\textbf{Step 4:}  Remove the AND from $A*(B+C')$ producing two child subexpressions $A$ and $(B+C')$ &
\textbf{Step 4:}  Remove the AND from $B'C$ producing two child subexpressions $B'$ and $C$ \\ \hline
...
\textbf{Figure}~\ref{fig:representationsBasic}: The circuit diagram for $F(A,B,C)=A(B+C')+B'C$.}
 \par}%
}
\par}\vspace{0.2cm}
\end{process}

Two simplications were made during the parsing process which deserve some attention
leaves and \textit{paraenthesis}.  When there the parsing process can go no
...
enables low precedence operators to be evaluated before higher precendence operators.
onsequently, when an expression consists of a single set of parenthesis containing a 
