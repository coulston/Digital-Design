...
\item{c) } $D_{\rm For/If} = Q_{\rm Wait2}*REQ' + Q_{\rm Inc}$
\item{d) } $Q_{\rm For/If} = D_{\rm Wait2}*REQ' + D_{\rm Inc}$
\item{e) } None of the above.
\end{description}
...
\item{\bf (1 pt.)}  Given the entity description below, which of the following 
VHDL statements correctly instantiates the multiplexer on page 2 (involved
in problems 4-7)?
...
\begin{description}
\item{a) } uut: mux generic map (4) port map(A, 2, L12, M);
\item{b) } uut: mux generic map (5) port map(y1, y0, s, f);
\item{c) } uut: mux generic map (4) port map (2, A, L12, M);
\item{d) } uut: mux generic map (5) port map (2, A, L12, M);
\item{e) } uut: mux generic map (5) port map (A, 2, L12, M);
\end{description}
...
\begin{description}
\item{a) 4, 5, 1, 4 (repeats)} 
\item{b) 4, 5, 0, 1, 4 (repeats)} 
\item{c) 4, 5, 6, 0, 1, 4 (repeats) } 
\item{d) 4, 5, 6, 0, 1, 2, 4 (repeats) } 
\item{e) 4, 5, 6, 7, 0, 1, 2, 3, 4 (repeats) }
\end{description}
