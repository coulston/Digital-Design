...
%\usepackage{zref}          % Enables the use of zref
%\usepackage{xr}            % Enables the use of cross-file references (includes xr-hyper)
\usepackage{xr-hyper}
\externaldocument[1-]{chapter01/chapter1} % Prefixes as we are including each chapter into itself, so xr would collide with local (I think)
...
\hypersetup{
    colorlinks=true,
    linkcolor=red,
    filecolor=cyan, % color of file links (xr). Cyan so you can double check
    menucolor=blue,
