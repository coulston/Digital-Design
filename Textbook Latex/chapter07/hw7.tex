...
\begin{enumerate}
                ...
                \begin{onlysolution}{\color{blue}
                        \begin{tabular}{p{2in}p{2in}}
                            \begin{tabular}{l@{}l} % @{} removes all automatic padding from the table
                                $Q_{red}$      & $=Set30 + Red   \,\&\, Rt$    \\
                                $Q_{yellow}\,$ & $=Set5  + Yellow\,\&\, Yt$    \\ % Manually creating alignment removes the spacing around symbols, so artificially add it back on longest line
                                $Q_{green}$    & $=Set25 + Green \,\&\, Gt$    \\
                            \end{tabular}
                            &
                            \begin{tabular}{l@{}l}
                                $Z_{Rlight}$   & $= Red$ \\
    ...
    \item \textbf{ (12 pts.)}
        ...
        \begin{figure}[ht]
            \center{\scalebox{0.8}{\includegraphics{Prob7-9}}}
        ...
        \begin{onlysolution}  \textbf{Solution} \itshape{
            ...
            \textbf{MIE}
            \begin{align*}
                Straight &= Reset + Straight(ls'*rs') + Left(ls'*rs') + Right(ls'*rs') + OnLine(ls+rs)\\
                Left     &= Straight(ls*rs') \\
                Right    &= Straight(ls'*rs) \\
                OnLine   &= Left(ls*rs) + Right(ls*rs) + Straight(ls*rs) + OnLine(ls*rs)
            \end{align*}
            \textbf{OE}
            \begin{align*}
                lm &= Reset + Straight + Right + OnLine\\
                rm &= Reset + Straight + Left  + OnLine
            \end{align*}
        \end{onlysolution}
    ...
    \item \textbf{ (16 pts.)}
        ...
        \begin{onlysolution}  \textbf{Solution} \itshape{
                ...
                \textbf{MIE}
                \begin{align*}
                    Straight &= Reset + Straight(ls'*rs') + Left(ls'*rs') + Right(ls'*rs') + Straight(L)\\
                    Left     &= Straight(ls*rs') \\
                    Right    &= Straight(ls'*rs) \\
                    OnLine   &= Left(ls*rs) + Right(ls*rs) + Straight(ls*rs) + OnLine(ls*rs)\\
                    Up       &= OnLine(ls+rs) \\
    ...
    \item \textbf{ (24 pts.)}
        ...
        \begin{onlysolution}  \textbf{Solution} \itshape{
                ...
                \end{enumerate}
                \filbreak % looks bad with this. Looks worse without
                \begin{figure}[ht]
