...
In otherwrds, what happen when N=0 and D = 0.  If $N'D'$ is true, this
means no money was deposited, then the total amount of money
deposited does not change and consequently, we should not change 
state.  These self arcs are present on every state, except 35\textcent , but are omitted from the state diagram in the interests of clarity.

Why is the $N'D'$ self arc not included on the 35\textcent~state?
Well the FSM spends one clock cycle in this state dispensing a single soda, before 
transitioning back to state 0\textcent.  Hence, the FSM must go from
the 35\textcent~state to the 0\textcent~state regardless of the input conditions;
an unconditional transition.
By convention we do not put labels on unconditional transitions and
understand that the FSM will only spend a single clock cycle in a state that
has an unconditional arc leaving it.
...
Let's write the memory input equation for the flip flop
in charge of the 15\textcent~state.  To do this ask yourself how you
could get into this state.  There are three options:
You could be in state 5\textcent~and get a dime as an input
You could be in state 10\textcent~and get a nickel as an input
You could be in state 15\textcent~and get no nickel or dime input.
Remember that you will never get the input of a nickel and dime simultaneously.
Hence the MIE for state 5\textcent~is
$D_{15} = Q_{5}D+ Q_{10}N + Q_{15}N'D'$ 
The rest of the MIEs are listed below.
